...
2. persists one unique operation per mutation kind before the HTTP call;
3. fingerprints the exact request and refuses changed redispatch under the same intent;
4. includes `solidus:<store-code>:<shipment-number>:1` as Delivery `orderNo` and external reference;
5. never holds a database transaction across HTTP;
6. treats timeout, transport ambiguity, or malformed mutation responses as unknown rather than failed;
7. searches Delivery by the stable merchant reference before any operator considers another booking.
...
- [Delivery REST shipments](https://helpcenter.nshift.com/hc/en-us/articles/7471973291292-Integration-via-API-shipments)
- [Shipment Data API](https://helpcenter.nshift.com/hc/en-us/articles/360021438180-ShipmentData-API)
- [Delivery REST RAML](https://api.unifaun.com/rs-extapi/v1/documentation/root-resource-v1.raml)
